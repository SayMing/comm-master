 ...
 * @date 2024-04-04
 */
public class ExpandSplitIter extends ComputeIter<String> implements Serializable {
    ...
    private final boolean containSplit;
    /**
     * 上一次的字符长度
     */
    ...
    public static List<String> split(String text, String reg, int limit, boolean isTrim, boolean ignoreEmpty, boolean containSplit) {
        Assert.notNull(text, "Text must be not null!");
        if (text.length() < 1) {
            return new ArrayList<>(0);
        }
        final Pattern pattern = PatternPool.get(reg);
        final ExpandSplitIter splitIter = new ExpandSplitIter(text, new PatternFinder(pattern), limit, ignoreEmpty, containSplit);
     ...
     */
    public ExpandSplitIter(CharSequence text, TextFinder separatorFinder, int limit, boolean ignoreEmpty, boolean containSplit) {
        Assert.notNull(text, "Text must be not null!");
        this.text = text.toString();
        this.finder = separatorFinder.setText(text);
        this.limit = limit > 0 ? limit : Integer.MAX_VALUE;
        this.ignoreEmpty = ignoreEmpty;
        this.containSplit = containSplit;
    }
    ...
    protected String computeNext() {
        ...
        if (start < 0) {
            ...
            if (offset <= text.length()) {
                if(containSplit) {
                    result = text.substring(offset - preLength);
                }else {
        ...
        if(containSplit) {
            result = text.substring(offset - preLength, start);
        }else {
